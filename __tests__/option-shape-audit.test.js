 ...
 *
 * Today (2) is "nothing at all" for every case below. That is deliberate for
 * v2-stable options — throwing on a minor upgrade is the breaking change #11
 * exists to avoid — but it is not free: three of these cases silently leave a
 * file SERVED that the operator plainly meant to hide, and one silently
 * disables compression entirely. Those are called out individually.
 *
 * When 6.0.0 promotes these to warnings or throws, this file is the inventory
 * of what has to change, and each failing assertion names the option.
 */
...
describe('hidden — a wrong shape fails OPEN, in silence', () => {
    ...
    test('hidden.dotFiles: "hidden" (string instead of { default })→ .env STAYS SERVED', async () => {
        ...
        expect(logger.warns).toEqual([]); // #11: nothing is reported
    });

    test('hidden.dotFiles.blacklist: ".env" (string instead of array) → .env STAYS SERVED', async () => {
    ...
    test('hidden.alwaysHide: "*.key" (string instead of array) → secret.key STAYS SERVED', async () => {
        const { server, logger } = makeServer({ hidden: { alwaysHide: '*.key' } });
        expect((await supertest(server).get('/secret.key')).status).toBe(200);
        expect(logger.warns).toEqual([]);
    });

    test('hidden: "yes" (not an object) → the whole namespace is discarded', async () => {
