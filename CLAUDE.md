...
Current version: **5.0.0** (see `docs/CHANGELOG.md`). Peer-depends on `koa` (`^2.16.4 || >=3.1.2`); the only runtime dependency is `mime-types`. Requires Node `>=18`.
...
- **`docs/PERFORMANCE_TUNING.md`** — RAM/CPU sizing guide for the caches and compression paths; linked from README.
- **`docs/revisione_codice_v4.3.md`** — findings register from the 2026-07-14 full code review (in Italian). All 16 findings are now **resolved** (`[x]`); the last three (#9 `hideExtension.redirect` strict validation, #10 `template.ext` leading-dot equivalence / unified suffix matching) shipped in **5.0.0**. Kept as the record of what was fixed and why. Each finding has a checkbox in its index — the convention is to tick it (`[x]`) when resolved or consciously closed. The **next** review should open a fresh `docs/revisione_codice_vX.Y.md` register following this same format.
