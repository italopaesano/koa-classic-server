...
| `compression.maxFileSize: 10 MB` | Unbounded RAM/CPU on the buffered compression path (whole file in RAM + brotli Q11 for a multi-GB text file). Above the cap, files are still compressed via the bounded-RAM streaming mode, whose output is teed into the compressed cache when it fits (≤ ¼ of that cache's `maxSize`) so repeat requests are RAM hits |
