...
- **Behavior** (default `true`): `GET /dir` → `301 /dir/` (query string preserved, percent-encoding preserved, `urlPrefix` included); `GET /file/` → `404`. Root `/` never redirects. Redirects only fire when the directory would actually render (listing enabled), so a listing-disabled directory 404s directly without a redirect-to-404. `useOriginalUrl: false` builds the `Location` from the client's original URL. A `//host`-style path is collapsed to a single leading slash (no protocol-relative open redirect).
